...
export function Poster({title,index=0}:{title:Title;index?:number}) {
 ...
 return <article className="poster-card" style={{'--delay':`${index*35}ms`} as React.CSSProperties}>
  ...
  </button><span className="media-pill">{kindLabel(title)}</span><button className={`poster-save glass ${saved?'is-saved':''}`} onClick={()=>saved?remove(title):save(title)} disabled={busyIds.has(title.id)} aria-label={`${saved?'Remove':'Save'} ${title.title}`} aria-pressed={saved}>{saved?<Check size={16}/>:<Plus size={18}/>}</button></div>
