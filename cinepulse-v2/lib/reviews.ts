...
function mapped(row:any): Review { return {id:row.id,userId:row.user_id,name:row.name,titleId:row.title_id,titleName:row.title_name,body:row.body,rating:row.rating===null?null:Number(row.rating),spoiler:Boolean(row.spoiler),kind:row.kind,createdAt:row.created_at}; }
const reviewSelect=`SELECT r.*,u.name FROM reviews r JOIN users u ON u.id=r.user_id`;
export function community(): Review[] { return (db().prepare(`${reviewSelect} ORDER BY r.created_at DESC LIMIT 50`).all() as any[]).map(mapped); }
export function titleReviews(titleId:string): Review[] { return (db().prepare(`${reviewSelect} WHERE r.title_id=? ORDER BY r.created_at DESC`).all(titleId) as any[]).map(mapped); }
export async function putReview(user: User,titleId:string,input:any): Promise<void> {
  ...
  const stamp=now(); db().prepare(`INSERT INTO reviews(id,user_id,title_id,title_name,body,rating,spoiler,kind,created_at) VALUES(?,?,?,?,?,?,?,?,?) ON CONFLICT(user_id,title_id) DO UPDATE SET title_name=excluded.title_name,body=excluded.body,rating=excluded.rating,spoiler=excluded.spoiler,kind=excluded.kind,created_at=excluded.created_at`).run(randomUUID(),user.id,title.id,title.title,normalizedBody, rating, spoiler?1:0,kind,stamp);
}
export function deleteReview(user:User,titleId:string):void { const result=db().prepare('DELETE FROM reviews WHERE user_id=? AND title_id=?').run(user.id,titleId) as any; if (!result.changes) throw notFound('Review not found'); }
