...
function forecast(row:any): Forecast { return {titleId:row.title_id,choice:row.choice,confidence:Number(row.confidence),reason:row.reason,createdAt:row.created_at,updatedAt:row.updated_at}; }
export function isForecastOpen(releaseDate:string|null, today=new Date().toISOString().slice(0,10)):boolean { return !!releaseDate && isValidDate(releaseDate) && isValidDate(today) && releaseDate > today; }
const isOpen=isForecastOpen;
export async function getPulse(titleId:string,user?:User|null):Promise<Pulse> {
  const title=await titleById(titleId); const d=db();
  const rows=d.prepare('SELECT choice,COUNT(*) count FROM forecasts WHERE title_id=? GROUP BY choice').all(title.id) as any[];
  const hit=Number(rows.find(x=>x.choice==='hit')?.count||0), flop=Number(rows.find(x=>x.choice==='flop')?.count||0), count=hit+flop;
  ...
  const byDate=new Map<string,{count:number;hit:number}>(); for (const e of events) { const date=String(e.created_at).slice(0,10); const item=byDate.get(date)||{count:0,hit:0}; item.count++; if(e.choice==='hit')item.hit++; byDate.set(date,item); }
  ...
  const open=isOpen(title.releaseDate); return {count,hit,flop,hitShare:count ? hit/count : null,interval:wilson(hit,count),stage:count===0?'no-data':count<10?'early':count<50?'growing':'established',history:Array.from(byDate.entries()).map(([date,v])=>({date,...v})),myForecast:my?forecast(my):null,forecastOpen:open,target};
}
export async function putForecast(user:User,titleId:string,input:any):Promise<void> {
  const title=await titleById(titleId); if (!isOpen(title.releaseDate)) throw bad('Forecasts are closed for this title');
...
export async function myForecasts(user:User):Promise<{forecast:Forecast;title:Awaited<ReturnType<typeof titleById>>}[]> {
  const rows=db().prepare('SELECT * FROM forecasts WHERE user_id=? ORDER BY updated_at DESC').all(user.id) as any[]; const out=[] as {forecast:Forecast;title:Awaited<ReturnType<typeof titleById>>}[];
  for (const row of rows) { try { out.push({forecast:forecast(row),title:await titleById(row.title_id)}); } catch { if(row.title_json) { try { out.push({forecast:forecast(row),title:JSON.parse(row.title_json)}); } catch {} } } }
  return out;
}
